...
![Spring Boot Banner](./images/20200610114804.png)

但有时我们需要将默认横幅更换掉，换上自己的或者公司的LOGO，我们就需要知道 Spring Boot 的 Banner 究竟是不是可配置的。

# 源码分析

现在我们分析一下Spring Boot在启动时必须经过的方法 `run()`：

![Spring Boot Application Run](./images/20200611124349.png)
...
接着继续往下查看，可从 `getBanner()` 方法中得知，`banner` 存储的内容会是图像和文本，如果都没有获到，会先判断`fallbackBanner` 是否为空，为空的话在使用默认 `banner`。
...
我们获取 `IMAGE_EXTENSION` 数组得知`banner` 获取的图像后缀的数组内容：
...
而获取 `banner.txt` 是否图像横幅同时进行的。在 `getBanner()` 中有一个 `banner.hasAtLeastOneBanner()` 判断，至少要有一个横幅存在才会返回 `banner` 存储。
...
这样，一个 `Banner` 的打印流程就完成了。下面我们来配置自己的 `Banner`图 。

# Banner配置

首先我们设置一个 `banner.png` 文件并将它存储在 `src/main/resources` 中：

![Spring Boot Banner Png](./images/20200611214550.png)

运行该Spring Boot后，在控制台输出：

![Spring Boot Png Console](./images/20200611215135.png)

在设置一个 `banner.jpg` 文件：

![Spring Boot Banner Jpg](./images/20200611215415.png)

运行后可以看到：

![Spring Boot Banner Jpg Console](./images/20200611232515.png)

在设置一个 `banner.gif` 文件：

![Spring Boot Banner Gif](./images/20200611235032.png)

运行后可以看到：

![Spring Boot Banner Gif Console](./images/20200611235243.png)

在设置一个 `banner.txt`文件：

![Spring Boot Banner](./images/20200611235423.png)

再次启动项目，可以看到如下内容：

![Spring Boot Banner](./images/20200611235751.png)
...
![Spring Boot Banner Color](./images/20200612001649.png)

输出的结果：

![Spring Boot Banner Color Console](./images/20200612001756.png)
...
![Spring Boot Banner Expression](./images/20200612123525.png)

效果如下：

![Spring Boot Banner Expression Console](./images/20200612123317.png)
...
如果Spring Boot自带的`Banner`类不适合，我们可以定制一个实现了 `Banner` 接口的 `Banner` 类，来满足我们的需求：
...
就可以了，如果想对 `Banner` 进行扩展，只需要对 `SampleBanner` 增加功能即可。
